...
export function printLegalBulk({
  ...
  // Build full HTML for all legal pages
  let pagesHtml = '';
        ...
        <div class="duty-card">
          ...
          <div class="card-header">
            ...
            <div class="header-titles">
              <div class="title-main">${eventTitle}</div>
              <div class="title-sub">${eventSubtitle}</div>
            </div>
            <img src="/badge.png" class="badge-icon" />
          </div>
          ...
          <div class="officer-box">
            ...
            <div class="officer-details">
              <div>
                ...
                <div class="officer-name">${duty.name || '-'}</div>
                <div class="officer-phone">📱 ${duty.mobile || '-'}</div>
              </div>
          ...
          <table class="card-table">
            <tbody>
              <tr>
                ...
                <td class="td-val-highlight">${duty.duty_place || '-'}</td>
              </tr>
              <tr>
                <td class="td-label">दिनाँक व समय</td>
                <td class="td-val font-bold">${duty.shift || '-'}</td>
              </tr>
              <tr>
                <td class="td-label">जोन / प्रभारी</td>
                <td class="td-val">${duty.zone || '-'} / ${duty.zonal_incharge || duty.zonal || '-'}</td>
              </tr>
              <tr>
                <td class="td-label">सेक्टर / प्रभारी</td>
                <td class="td-val">${duty.sector || '-'} / ${duty.sector_incharge || '-'}</td>
              </tr>
              ${activeBriefing ? `
              <tr class="row-highlight">
                <td class="td-label">ब्रीफिंग</td>
                <td class="td-val font-bold">${activeBriefing}</td>
              </tr>` : ''}
              ${activeNote ? `
              <tr class="row-highlight">
                <td class="td-label">विशेष नोट</td>
                <td class="td-val note-text font-bold">${activeNote}</td>
              </tr>` : ''}
            </tbody>
          </table>
          ...
          <div class="card-footer">
            <div class="footer-left">
              ...
              <div class="pass-id">PASS ID: ${duty.id || '-'}</div>
            </div>
            <div class="footer-right">
              ${signHtml}
              <div class="signatory-name">${signatoryText}</div>
            </div>
          </div>
        </div>
      `;
  ...
  const fullHtml = `
    ...
    <head>
      <meta charset="utf-8">
      <title>${eventTitle} - Bulk Duty Pass (Legal 4-in-1)</title>
      <style>
        @page {
          size: legal portrait;
          margin: 6mm;
